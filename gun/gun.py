...
def init_game():
    """
    создаем необходимое для игры количество объектов-шариков
...
def init_main_window():
    global root, canvas, scores_text, scores_value, button1
    root = Tk()
    root.title = ("Пушка")
    scores_value = IntVar()
...
def timer_event(event):
    #for i in range(Ball.initial_number):
    for ball in balls:
        ball.fly()
    for shell in shells_on_fly:
        shell.fly()
    canvas.after(timer_delay, timer_event, None)
